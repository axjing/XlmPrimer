...
class LLMConfig:
    ...
    # generation & pool
    summary_type:str="cls_index" # 取<CLS> 位置做句子、分类表示
    ...
    reorder_and_upcast_attn:bool=False # 不重排、提升精度计算注意力    


    # special tokens
    bos_token_id:int=5026 # 开始符 token id，用于表示序列的开始
    eos_token_id:int=5026 # 介绍符 token id,用于表示序列的结束,默认与开始符相同
    pad_token_id:int|None=None # 填充 token id，用于填充到最大上下文长度

    # 结构开关
    add_cross_attention:bool=False # 是否添加跨注意力机制
    tie_word_embeddings:bool=True # 是否将 word embedding 层和 output 层的权重共享(GPT经典做法，为了省参数量)
